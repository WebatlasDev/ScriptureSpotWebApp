// Static Bible structure data - chapter and verse counts for all 66 books
// This eliminates API calls for navigation metadata, providing instant response
...
export const BIBLE_STRUCTURE: Record<string, BookStructure> = {
  // Old Testament
  'genesis': {
    chapters: 50,
...
export function getChapterCount(bookSlug: string): number {
  return BIBLE_STRUCTURE[bookSlug]?.chapters || 0;
}
...
export function getVerseCount(bookSlug: string, chapter: number): number {
  const book = BIBLE_STRUCTURE[bookSlug];
...
// Helper function to get all chapters for a book (mimics API response format)
export function getChaptersArray(bookSlug: string): number[] {
  const chapterCount = getChapterCount(bookSlug);
  return Array.from({ length: chapterCount }, (_, i) => i + 1);
}

// Helper function to get all verses for a chapter (mimics API response format)
export function getVersesArray(bookSlug: string, chapter: number): number[] {
  const verseCount = getVerseCount(bookSlug, chapter);
